    ...
    <main className="app-shell">
      ...
      {authentication.status !== 'error' && !activeSessionId && (
        <div className="home-grid">
          ...
          <section className="auth-card">{authentication.status === 'signedOut' ? <button onClick={() => void firebaseAuthentication.continueWithGoogle()}>{t('signInGoogle')}</button> : authentication.status === 'authenticated' && authentication.identity.kind === 'anonymous' ? <button onClick={() => void firebaseAuthentication.continueWithGoogle()}>{t('linkGoogle')}</button> : null}{googleOutcome.status === 'credential-already-in-use' && <p role="status">{t('linkedAccount')}</p>}</section>
        </div>
      )}
