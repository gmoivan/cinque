...
| Secrets and IAM | Ready with accepted risk | Nine HTTP services have individual `allUsers` invoker bindings, cleanup remains private, and the runtime has conditional `datastore.user` access only to staging `(default)`. Deployer Service Account uses granular roles. (Experimentos 2 y 3 demostraron que separar Auth bootstrap del deploy rutinario permitió la remoción exitosa de `serviceusage.serviceUsageAdmin` y `firebase.admin`, validando las actualizaciones de Functions con `cloudfunctions.admin` y eliminando estos roles excesivamente permisivos del deployer en staging). |
