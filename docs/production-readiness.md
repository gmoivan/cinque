...
| Secrets and IAM | Ready with accepted risk | Nine HTTP services have individual `allUsers` invoker bindings, cleanup remains private, and the runtime has conditional `datastore.user` access only to staging `(default)`. Deployer Service Account retains `firebase.admin` as a temporary/accepted staging risk pending narrower deploy role discovery and refactor. (Experimento 2 demostró que separar Auth bootstrap del deploy rutinario permitió la remoción exitosa de `serviceusage.serviceUsageAdmin`, reduciendo significativamente los privilegios del deployer en staging). |
