...
`npm run test:create-session` (also available as `npm run test:join-session`) builds the repository-local Node 22 Functions project and runs real Create/Join flows against the Auth, Firestore, and Functions emulators. `npm run test:start-session` runs the Start flow, retry, and Join-vs-Start transaction race against those same emulators. `npm run test:record-score` covers idempotent score recording and winner detection, including concurrent target crossings. The Emulator Suite includes its UI and uses `demo-cinque` only.
