...
export async function resolveScoreReportRecord(firestore: Firestore, uid: string, input: ValidResolveScoreReportInput): Promise<ResolvedScoreReport> {
  return firestore.runTransaction(async (transaction) => {
    ...
    const [sessionSnap, playerSnap, reportSnap, resolutionSnap, playersSnap, correctionsSnap] = await Promise.all([transaction.get(sessionRef), transaction.get(playerRef), transaction.get(reportRef), transaction.get(resolutionRef), transaction.get(sessionRef.collection('players')), transaction.get(sessionRef.collection('scoreCorrections'))])
